...
import math 
import cv2
import gymnasium  #changed from gym to gymnasium
...
class CarEnv(gymnasium.Env): #changed from gym to gymnasium
	...
	def step(self, action):
		...
		if self.episode_start + SECONDS_PER_EPISODE < time.time():
			...
			self.cleanup()
		return cam/255.0, reward, done, done,{}	#curly brackets - empty dictionary required by SB3 format
	...
	def reset(self,seed=None):
		...
		return self.front_camera/255.0, dict()
